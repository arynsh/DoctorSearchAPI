...
export class MedicalSearch {
  async searchUsingSymptom(symptom) {
    try {
      let symptomresponse = await fetch(`https://api.betterdoctor.com/2016-03-01/doctors?query=${symptom}&location=wa-seattle&skip=0&limit=10&user_key=${process.env.API_KEY}`);
